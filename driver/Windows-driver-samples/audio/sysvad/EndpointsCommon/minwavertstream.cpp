...
--*/
{
    ...
    if (m_bCapture)
    {
        VamAudioTransportCloseCaptureStream(this);
    }
...
--*/
{
    ...
    if (!Capture_)
    {
        VamAudioTransportSetRenderFormat(&m_pWfExt->Format);
    }
...
--*/
{
    ...
    while (ByteDisplacement > 0)
    {
        ...
        VamAudioTransportReadCaptureFormat(
            ...
            IsEqualGUIDAligned(m_pWfExt->SubFormat, KSDATAFORMAT_SUBTYPE_IEEE_FLOAT) ? TRUE : FALSE);
...
--*/
{
    ...
    {
        ULONG runWrite = min(ByteDisplacement, m_ulDmaBufferSize - bufferOffset);
        VamAudioTransportWriteRender(m_pDmaBuffer + bufferOffset, runWrite);
