...
DWORD g_DisableToneGenerator = 1;  // VirtualAudioMix defaults to silence.
...
--*/
    ...
    DriverObject->MajorFunction[IRP_MJ_CREATE] = VamAudioTransportDispatchCreateClose;
    DriverObject->MajorFunction[IRP_MJ_CLOSE] = VamAudioTransportDispatchCreateClose;
    DriverObject->MajorFunction[IRP_MJ_DEVICE_CONTROL] = VamAudioTransportDispatchDeviceControl;

    ntStatus = VamAudioTransportInit(DriverObject);
    if (!NT_SUCCESS(ntStatus))
    {
        DPF(D_ERROR, ("VamAudioTransportInit failed, 0x%x", ntStatus));
